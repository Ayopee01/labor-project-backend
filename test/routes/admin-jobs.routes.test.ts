...
test("GET /api/admin/vehicle-jobs/operations includes shirt_number per assigned worker from MasterWorker.coat_no, including null", async () => {
  const { token } = await loginJobAdmin(9932);
  const job = addDispatchableJob(995, 1);
  const worker = addWorker(9951);
  worker.coat_no = "42";
  const noShirtWorker = addWorker(9952);
  noShirtWorker.coat_no = null;
  addPendingAssignment(19951, job.id, worker.id);
  addPendingAssignment(19952, job.id, noShirtWorker.id);
  ...
  assert.equal(workerRow.shirt_number, "42");
...
test("GET /api/admin/vehicle-jobs/history/daily-stall-fees narrows available_products by package_code without narrowing available_packages, independent of pagination", async () => {
  const { token } = await loginJobAdmin(96018);
  const job = addDispatchableJob(9607, 1);
  const ticket = addTicketForVehicleJob(job.id, 196070); // สร้าง product Apple/fruit และ Cabbage/vegetable ให้อัตโนมัติ
  const appleProductId = ticket.id * 10 + 1;
  const applePackageCode = state.ticketProducts.find((item) => item.id === appleProductId)!.packageCode;

  addTicketProductFinancial(960701, appleProductId, {
    finalized_at: bangkokDateToUtcIso("2026-08-10", 9),
  });
  addTicketProductFinancial(960702, ticket.id * 10 + 2, {
    finalized_at: bangkokDateToUtcIso("2026-08-10", 10),
  });

  const packageFilterResponse = await server.request(
    "GET",
    `/api/admin/vehicle-jobs/history/daily-stall-fees?date_from=2026-08-01&date_to=2026-08-31&package_code=${encodeURIComponent(applePackageCode)}&page=1&limit=1`,
    { token },
  );

  assert.equal(packageFilterResponse.status, 200, JSON.stringify(packageFilterResponse.body));
  // package_code=fruit (ของ Apple) ต้อง narrow available_products เหลือแค่ Apple
  assert.deepEqual(
    packageFilterResponse.body.available_products.map((item: { productCode: string }) => item.productCode),
    [state.ticketProducts.find((item) => item.id === appleProductId)!.productCode],
  );
  // แต่ available_packages ต้องไม่ narrow ตัวเอง (ยังเห็นทั้ง fruit และ vegetable)
  assert.equal(packageFilterResponse.body.available_packages.length, 2);

  // options ต้องไม่เปลี่ยนตาม page/limit — page 2 ของช่วงข้อมูลเดียวกันต้องให้ available_* ชุดเดิม
  const page2Response = await server.request(
    "GET",
    `/api/admin/vehicle-jobs/history/daily-stall-fees?date_from=2026-08-01&date_to=2026-08-31&page=2&limit=1`,
    { token },
  );

  assert.equal(page2Response.status, 200, JSON.stringify(page2Response.body));
  assert.equal(page2Response.body.available_products.length, 2);
  assert.equal(page2Response.body.available_packages.length, 2);
});

test("GET /api/admin/vehicle-jobs/history/daily-stall-fees returns 200 with empty Data and zero summary when product_code and package_code do not co-occur on any finalized row", async () => {
  const { token } = await loginJobAdmin(96019);
  const job = addDispatchableJob(9608, 1);
  const ticket = addTicketForVehicleJob(job.id, 196080); // สร้าง product Apple/fruit และ Cabbage/vegetable ให้อัตโนมัติ
  const appleProductCode = state.ticketProducts.find((item) => item.id === ticket.id * 10 + 1)!.productCode;
  const cabbagePackageCode = state.ticketProducts.find((item) => item.id === ticket.id * 10 + 2)!.packageCode;

  addTicketProductFinancial(960801, ticket.id * 10 + 1, {
    finalized_at: bangkokDateToUtcIso("2026-08-10", 9),
  });
  addTicketProductFinancial(960802, ticket.id * 10 + 2, {
    finalized_at: bangkokDateToUtcIso("2026-08-10", 10),
  });

  // Apple ไม่เคยอยู่คู่กับ package_code ของ Cabbage ("vegetable") ในแถว finalize ไหนเลย
  const response = await server.request(
    "GET",
    `/api/admin/vehicle-jobs/history/daily-stall-fees?date_from=2026-08-01&date_to=2026-08-31&product_code=${encodeURIComponent(appleProductCode)}&package_code=${encodeURIComponent(cabbagePackageCode)}`,
    { token },
  );
  ...
  assert.equal(response.body.summary.confirmed_quantity_total, "0.00");
  assert.equal(response.body.summary.stall_fee_total, "0.00");
});
...
test("GET /api/admin/vehicle-jobs/history/daily-stall-fees returns 200 with zero summary when nothing matches", async () => {
  const { token } = await loginJobAdmin(96015);

  const response = await server.request(
    "GET",
    "/api/admin/vehicle-jobs/history/daily-stall-fees?date_from=2020-01-01&date_to=2020-01-31",
    { token },
  );
  ...
  assert.equal(response.body.summary.confirmed_quantity_total, "0.00");
  assert.equal(response.body.summary.stall_fee_total, "0.00");
  assert.deepEqual(response.body.available_products, []);
  assert.deepEqual(response.body.available_packages, []);

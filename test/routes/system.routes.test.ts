...
import {
  cleanupRateLimitBucketsForTest,
...
test("rate limiter targets auth/admin routes but not health or LINE webhook", async () => {
  ...
  const firstHealth = await server.request("GET", "/health");
  const secondHealth = await server.request("GET", "/health");

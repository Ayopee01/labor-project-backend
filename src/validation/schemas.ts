...
// Format Gate client id that may be generated by the system or provided by Admin.
...
// Format ISO 8601 date-time with timezone offset, used for TicketCreatedAt from Gate.
const dateTimeString = trimmedString.pipe(
  z.iso.datetime({ offset: true, error: "Must use ISO 8601 date-time format." })
);

// Format time only in HH:mm for daily worker shift schedules.
const timeString = trimmedString.pipe(
  z.iso.time({ precision: -1, error: "Must use HH:mm format." })
);

// Format worker shift preset number: 1 = morning, 2 = evening.
...
// Format optional Gate client id for create body; empty string means auto-generate.
...
// Format Gate client status from Admin Settings payloads.
const gateClientStatusSchema = z.enum(GATE_CLIENT_STATUSES);

// Format Gate client status with active default for create body.
const defaultGateClientStatusSchema = z.preprocess(
  emptyStringToUndefined,
  gateClientStatusSchema.default("active")
);

// Format optional Gate client status for PATCH body.
const optionalGateClientStatusSchema = z.preprocess(
  emptyStringToUndefined,
  gateClientStatusSchema.optional()
);

// Format worker nationality values used by worker code generation.
const workerNationalitySchema = z.enum(WORKER_NATIONALITIES);

// Format worker shirt type values used by worker code generation.
const workerShirtTypeSchema = z.enum(WORKER_SHIRT_TYPES);

// Format optional worker nationality for PATCH/profile update bodies.
const optionalWorkerNationalitySchema = z.preprocess(
  emptyStringToUndefined,
  workerNationalitySchema.optional()
);

// Format optional worker shirt type for PATCH/profile update bodies.
...
// Format FCM platform sent by Mobile during auth or push-token refresh.
...
// Schema body for Worker Mobile to register/refresh FCM token outside login.
